...
def extract_form_c(text: str, pages_text=None, debug=None) -> Dict[str, str]:
    lines = split_lines(text)
    fields: Dict[str, str] = empty_fields()

    deceased_windows = window_after_labels(
        lines, [r"decedent", r"deceased", r"family tree for", r"estate of"], max_lines=3
    )
    ...
    petitioner_windows = window_after_labels(
        lines,
        [r"affiant", r"petitioner", r"distributee", r"relationship to decedent", r"person completing form"],
        max_lines=3,
    )
    ...
    for snippet in window_after_labels(
        lines, [r"domicile", r"resided at", r"address of decedent", r"address at death"], max_lines=4
    ):
        dec_addr_candidates.extend(find_addresses(snippet))
    near_dom = find_address_near_keywords(text, ["resided at", "domicile", "family home", "decedent"])
    if near_dom:
    ...
    for snippet in window_after_labels(
        lines, [r"mailing address", r"address of affiant", r"address of petitioner", r"home address"], max_lines=4
    ):
        pet_addr_candidates.extend(find_addresses(snippet))
    near_pet = find_address_near_keywords(text, ["affiant address", "petitioner address", "mailing address"])
    if near_pet:
    ...
    value_windows = window_after_labels(lines, [r"improved real property", r"value of", r"market value"], max_lines=3)
